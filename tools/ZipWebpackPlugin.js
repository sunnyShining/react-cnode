...
ZipPlugin.prototype.apply = function(compiler) {
	...
	compiler.plugin('emit', function(compilation, callback) {
		...
		for (var nameAndPath in compilation.assets) {
			...
			var fixNameAndPath = nameAndPath;
			//修正压缩文件名
			if(fixNameAndPath.lastIndexOf('?') > -1){
				fixNameAndPath = fixNameAndPath.substring(0, fixNameAndPath.lastIndexOf('?'))
			}
			zipFile.addBuffer(
				Buffer.isBuffer(source) ? source : new Buffer(source),
				path.join(pathPrefix, fixNameAndPath),
				options.fileOptions
			);
		}
		...
		zipFile.outputStream.on('end', function() {
			...
			var outputPath = options.outpath || compilation.options.output.path;//改写源代码，使其打包路径可自定义
